...
class GINetConvLayer(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        xcol = self.fc(x[col])
        xrow = self.fc(x[row])
        print(f"xcol  {xcol.shape}")
        print(f"xrow  {xrow.shape}")
        ...
        print(f"alpha {F.leaky_relu(alpha).shape}")
        alpha_equ = torch.mul(edge_attr,F.leaky_relu(alpha))
        print(f"alpha_1 {alpha_equ.shape}")
        alpha = F.softmax(alpha_equ, dim=1)
        h = alpha * xcol
...
class GINet(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        # first conv block
        data_ext.x = act(
            self.conv1_ext(data_ext.x, data_ext.internal_edge_index, data_ext.internal_edge_attr)
        )
        ...
        # second conv block
        data_ext.x = act(
            self.conv2_ext(data_ext.x, data_ext.internal_edge_index, data_ext.internal_edge_attr)
        )
